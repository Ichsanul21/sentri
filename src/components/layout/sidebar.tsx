...
const groupSections = [
  ...
  { start: 1, end: 5, label: "Management" },
  { start: 5, end: 6, label: "Monitoring" },
  { start: 6, end: 7, label: "Analytics" },
  { start: 7, end: 8, label: "Account" },
  { start: 8, end: 9, label: "System" },
];
...
export function Sidebar() {
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      <div key={item.href}>
        <div
          className={`flex items-center min-h-[36px] px-4 mx-2 rounded transition-all cursor-pointer ${
            active
          ...
          onClick={() => {
            if (!collapsed && hasSub) toggleExpand(item.href);
          }}
        >
        ...
        {!collapsed && hasSub && isExpanded && (
          <div className="ml-5 border-l border-hairline-violet/30 mt-0.5 mb-0.5">
            {item.sub!.map((sub) => {
  ...
  return (
    <aside
      className={`bg-surface-night border-r border-hairline-violet flex flex-col transition-all duration-200 ease overflow-hidden ${
        collapsed ? "w-16" : "w-56"
      }`}
    >
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin py-2">
        {groupSections.map((section) => (
          <div key={section.label} className="mb-1.5">
            ...
            {navItems.slice(section.start, section.end).map((item) => renderNavItem(item))}
          </div>
        ))}
      </nav>
      ...
      <button
        ...
        className="flex items-center justify-center h-8 mx-2 mb-2 rounded-md hover:bg-accent-violet-mid/20 transition-colors text-on-dark-muted"
      >
