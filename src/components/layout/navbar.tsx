...
import { Bell, Search, User, LogOut, CheckCheck, X, Settings } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    <header className="h-14 border-b border-hairline-violet/50 flex items-center justify-between px-6 bg-surface-canvas-dark">
      ...
      <div className="flex items-center gap-3">
        <div ref={notifRef} className="relative">
          <button
            ...
            className="relative p-2 rounded-md hover:bg-accent-violet-mid/20 transition-colors text-on-dark-muted hover:text-on-primary"
          >
            <Bell size={18} />
            {unreadCount > 0 && (
          ...
          {showNotifications && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-surface-night border border-hairline-violet rounded-xl shadow-2xl z-50 overflow-hidden">
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.slice(0, 20).map((n) => (
                    <div
                      ...
                      className={`px-4 py-3 border-b border-hairline-violet/20 cursor-pointer transition-colors hover:bg-accent-violet-mid/10 ${!n.read ? "bg-accent-lime/4" : ""}`}
                    >
        ...
        <div ref={userRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 p-2 rounded-md hover:bg-accent-violet-mid/20 transition-colors text-on-dark-muted hover:text-on-primary"
          >
            <User size={18} />
          ...
          {showUserMenu && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-surface-night border border-hairline-violet rounded-xl shadow-2xl z-50 overflow-hidden">
