...
export function Card({ variant = "widget-dark", className = "", children, ...props }: CardProps) {
  const base = "rounded-lg p-4 transition-colors duration-300";
  const variants: Record<string, string> = {
    ...
    "feature-dark": "bg-ink-deep text-on-primary rounded-[18px] p-8",
    "spotlight": "bg-accent-violet-deep text-on-primary rounded-[18px] p-8",
  };
  return (
...
export function WidgetCard({ children, title, className = "", ...props }: Omit<CardProps, "title"> & { title?: ReactNode }) {
  return (
    <Card variant="widget-dark" className={`flex flex-col animate-fade-in-up card-hover ${className}`} {...props}>
      {title && (
        <div className="flex items-center justify-between mb-2 pb-2 border-b border-hairline-violet/50">
          <h3 className="text-[16px] font-semibold leading-[1.25]">{title}</h3>
